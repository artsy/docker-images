...
# set to enable management SSL
: "${RABBITMQ_MANAGEMENT_SSL_ENABLED:=}"

# set to specify rabbitmq domain
: "${RABBITMQ_DOMAIN:=}"

# set to enable cluster partition handling
...
if [ "$1" = 'rabbitmq-server' ] && [ "$shouldWriteConfig" ]; then
	...
	if [ -n "$RABBITMQ_CLUSTER_PARTITION_HANDLING" ]; then
		rabbitConfig+=("{ cluster_partition_handling, $RABBITMQ_CLUSTER_PARTITION_HANDLING }")
	fi
	...
	if [ "$haveManagementSslConfig" ] && [ -n "$RABBITMQ_MANAGEMENT_SSL_ENABLED" ]; then
		IFS=$'\n'
...
if [ -n "$RABBITMQ_DOMAIN" ]; then
	export RABBITMQ_NODENAME=rabbit@$HOSTNAME$RABBITMQ_DOMAIN
else
